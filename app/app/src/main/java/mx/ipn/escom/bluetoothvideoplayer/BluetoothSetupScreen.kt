...
fun BluetoothSetupScreen(
    ...
    var receivedVideoTitle by rememberSaveable {
        mutableStateOf("")
    }
            ...
            when {
                ...
                role == BluetoothRole.SERVER -> {
                    ...
                    ClientContent(
                        ...
                        receivedVideoTitle = receivedVideoTitle,
...
private fun ClientContent(
    ...
    val context = LocalContext.current
    val playbackLibrary = remember(context) {
        PlaybackLibrary(context.applicationContext)
    }

    var favoriteRevision by remember {
        mutableStateOf(0)
    }

    val favoriteRevisionValue =
        favoriteRevision
    ...
    if (isConnected) {
        ...
        if (privateMode) {
            MessageCard(
                title = "Modo privado activado",
                message =
                    "Las reproducciones nuevas y los favoritos no se guardarán mientras permanezca activo."
            )
        ...
        searchResults.forEach { video ->
            ...
            Card(modifier = Modifier.fillMaxWidth()) {
                Column(
                    ...
                    val favoriteKey =
                        PlaybackLibrary.youtubeKey(
                            video.videoId
                        )

                    val isFavorite = remember(
                        video.videoId,
                        favoriteRevisionValue
                    ) {
                        playbackLibrary.isFavorite(
                            favoriteKey
                        )
                    }
                    ...
                    OutlinedButton(
                        onClick = {
                            playbackLibrary.toggleFavorite(
                                key = favoriteKey,
                                title = video.title,
                                subtitle =
